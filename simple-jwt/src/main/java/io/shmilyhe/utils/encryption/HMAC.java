...
public class HMAC {
	...
	public static String hmac(String salt,Hash h,String ...password){
		HMAC mac=	new HMAC(salt.getBytes(),h);
		for(int i=0;i<password.length-1;i++){
			mac.write(password[i].getBytes());
		}
		return Bytes.toHexString(mac.sum(password[password.length-1].getBytes()));
	}
